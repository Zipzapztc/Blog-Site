...
class BindEmailForm(forms.Form):
    ...
    def clean_verification_code(self):
        verification_code = self.request.session.get('bind_email_code','')
        user_code = self.cleaned_data['verification_code']
        if not(verification_code!='' and verification_code==user_code):
            raise forms.ValidationError('验证码错误')
        return verification_code
...
class BindUserForm(forms.Form):
    ...
    def clean(self):
        ...
        oauth_type = self.request.session['oauth_type']
        if OAuthRelationship.objects.filter(user=user, oauth_type=oauth_type).exists():
            raise forms.ValidationError('该用户已经被绑定')
        return self.cleaned_data
